...
static uint16_t adcSonar_Distance;
...
static void adcSonar_ParamsInit(void)
{
	...
	xadc_irq_flag = ATOMIC_INIT(0);

	/* Enable interrupt and sequence mode */
	xadc_config.interrupt_enable = true;
	xadc_config.seq_channel_mask = 1 << 14;
	xadc_config.sequence_mode_en = true;

	/* Config 1 register */
	xadc_config.config1.B.averaging_sample = XADC_AVERAGING_16_SAMPLES;
	xadc_config.config1.B.bibolar_en = 1U;
	xadc_config.config1.B.channel = XADC_CHANNEL_VAUX14;
	xadc_config.config1.B.disable_average = 0u;
	xadc_config.config1.B.event_mode_en = 1U;

	/* Config 2 register */
	xadc_config.config2.B.overtemp_alm_dis = 1U;
	xadc_config.config2.B.alarm_ext_dis = 0U;
	xadc_config.config2.B.alarm_int_dis = 0U;
	xadc_config.config2.B.calib_en = XADC_SENSOR_OFFSET_CORR_EN | XADC_SENSOR_OFFSET_GAIN_CORR_EN;
	xadc_config.config2.B.channel_seq_mode = XADC_SINGLE_PASS_SEQ_MODE;

	/* Config 3 register */
	xadc_config.config3.B.power_down_en = 0u;
	xadc_config.config3.B.drp_clk_pre = XADC_DCLCK_DIV_4;
	/* Axi size */
	xadc_config.axi_size = 0x10000;

	/* Base address */
	xadc_config.xadc_base_address = 0x43C10000U;

	/* Interrupt */
	xadc_config.irq_handler = Xadc_IrqCb;

	/* Device string */
	xadc_config.device_string = xadc_dev_string;
}
...
static int __init adcsonar_init(void)
{
	...
	Xadc_Init(&xadc_config);
	/* Avoid starting a thread if the initialization is failed */
	if (Xadc_Base != NULL)
	{
		xadc_ktask = kthread_run(adcSonar_thread, NULL, "adcSonar_MainThread");
	}
